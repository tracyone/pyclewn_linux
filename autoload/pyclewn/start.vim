...
function s:init(debugger)
    ...
    echo "[Debug] ".g:pyclewn_python

    if !executable(g:pyclewn_python)
        throw "Error: '" . g:pyclewn_python . "' cannot be found or is not an executable."
    endif
...
function s:start(args, pdb_attach)
    ...
    let l:run_pyclewn = s:python ." ". s:fixed . l:tmpfile . " " . a:args
    if s:terminal == ""
